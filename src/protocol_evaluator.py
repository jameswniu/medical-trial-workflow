...
Guarantees that every criterion in the YAML has an evidence entry
(PASS, FAIL, or MAYBE) for each patient.
Handles duration checks for rules with 'minimum_duration'.
"""
        ...
        for _, row in self.patients.iterrows():
            ...
            for crit in structured:
                ...
                if ctype == "age":
                    ...
                    if self.notes.check_presence(patient_id, crit.get("concepts", [])):
                        evidence[key] = f"PASS (Family history: {crit['concepts'][0]})"
                        score += 1; passed += 1
                    else:
                        evidence[key] = f"FAIL (No family history of {crit['concepts'][0]})"
...
if __name__ == "__main__":
    ...
    patients = pd.DataFrame([
        ...
        {"patient_id": "C002", "date_of_birth": "1985-01-01", "is_smoker": True},
    ])
    labs = pd.DataFrame([
        {"patient_id": "C001", "lab_test_name": "HbA1c", "value": 7.2},
        {"patient_id": "C002", "lab_test_name": "HbA1c", "value": 9.1},
    ])
    notes = {
